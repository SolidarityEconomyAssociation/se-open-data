...
def get_rdfxml_for_virtuoso(x,y)
  get_rdfxml x, $config_map["GEN_VIRTUOSO_DIR"]+y
end
...
begin
  if !File.file?($config_map["VIRTUOSO_SCRIPT_LOCAL"])
    ...
    get_rdfxml_for_virtuoso(
      $config_map["ESSGLOBAL_URI"]+"vocab/",
      "essglobal_vocab.rdf"
    )
    get_rdfxml_for_virtuoso(
      $config_map["ESSGLOBAL_URI"]+"standard/organisational-structure",
      "organisational-structure.skos"
    )
    
    puts "Creating #{$config_map["VIRTUOSO_NAMED_GRAPH_FILE"]}"
    IO.write $config_map["VIRTUOSO_NAMED_GRAPH_FILE"], $config_map["GRAPH_NAME"]

    puts "Creating #{$config_map["VIRTUOSO_SCRIPT_LOCAL"]}"
    IO.write $config_map["VIRTUOSO_SCRIPT_LOCAL"], <<HERE
SPARQL CLEAR GRAPH '#{$config_map["GRAPH_NAME"]}';
ld_dir('#{$config_map["VIRTUOSO_DATA_DIR"]}','*.rdf',NULL);
ld_dir('#{$config_map["VIRTUOSO_DATA_DIR"]}','*.skos',NULL);
rdf_loader_run();
HERE
    
    puts "Transfering directory '#{$config_map["GEN_VIRTUOSO_DIR"]}' to virtuoso server '#{$config_map["DEPLOYMENT_SERVER"]}':#{$config_map["VIRTUOSO_DATA_DIR"]}"
                                                  
    system <<HERE
#{ssh} #{$config_map["DEPLOYMENT_SERVER"]} 'mkdir -p #{$config_map["VIRTUOSO_DATA_DIR"]}' &&
#{rsync} #{$config_map["GEN_VIRTUOSO_DIR"]} #{$config_map["DEPLOYMENT_SERVER"]}:#{$config_map["VIRTUOSO_DATA_DIR"]}
HERE
